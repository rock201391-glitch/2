...
function getCartItemImageSrc(image: string | null | undefined) {
  ...
  const storagePathMatch = normalized.match(
...
export default function Cart({
  ...
  return (
    <div className="h-full w-full bg-[#FBF7EF] flex flex-col">
      {/* Header */}
      <div className="flex items-center justify-between p-6 border-b border-[#E8E3D9]">
        ...
        <span
          className="text-sm font-bold bg-white rounded-full px-3 py-1"
          style={{ color: '#0F3A2B' }}
        >
          ({items.length})
        </span>
        ...
        {items.length === 0 ? (
          ...
          items.map((item) => {
            ...
            return (
              ...
              >
                {/* Product Image */}
                <div className="w-20 h-20 rounded-xl overflow-hidden flex items-center justify-center flex-shrink-0 bg-[#F8F7F2]">
                  {imageSrc ? (
                    <ImageWithFallback
                      src={imageSrc}
                      alt={item.name}
